...
{
    var aX = this.node.appendChild(strophe_js.Strophe.xmlElement('x', {"xmlns": "jabber:x:data", "type": "submit"}));
...
*/
    ...
    init: function(conn) {
        ...
        */
        ...
        strophe_js.Strophe.addNamespace('PUBSUB_SUBSCRIBE_OPTIONS',
        ...
        strophe_js.Strophe.addNamespace('PUBSUB_AUTO_CREATE',
                             strophe_js.Strophe.NS.PUBSUB+"#auto-create");
        strophe_js.Strophe.addNamespace('PUBSUB_PUBLISH_OPTIONS',
                             strophe_js.Strophe.NS.PUBSUB+"#publish-options");
        strophe_js.Strophe.addNamespace('PUBSUB_NODE_CONFIG',
                             strophe_js.Strophe.NS.PUBSUB+"#node_config");
        strophe_js.Strophe.addNamespace('PUBSUB_CREATE_AND_CONFIGURE',
                             strophe_js.Strophe.NS.PUBSUB+"#create-and-configure");
        strophe_js.Strophe.addNamespace('PUBSUB_SUBSCRIBE_AUTHORIZATION',
                             strophe_js.Strophe.NS.PUBSUB+"#subscribe_authorization");
        strophe_js.Strophe.addNamespace('PUBSUB_GET_PENDING',
                             strophe_js.Strophe.NS.PUBSUB+"#get-pending");
        strophe_js.Strophe.addNamespace('PUBSUB_MANAGE_SUBSCRIPTIONS',
                             strophe_js.Strophe.NS.PUBSUB+"#manage-subscriptions");
        strophe_js.Strophe.addNamespace('PUBSUB_META_DATA',
                             strophe_js.Strophe.NS.PUBSUB+"#meta-data");
        strophe_js.Strophe.addNamespace('ATOM', "http://www.w3.org/2005/Atom");

        if (conn.disco)
            conn.disco.addFeature(strophe_js.Strophe.NS.PUBSUB);
    ...
    */
    ...
    discoverNodes: function(success, error, timeout) {
        ...
        //ask for all nodes
        var iq = strophe_js.$iq({from:this.jid, to:this.service, type:'get'})
    ...
    getConfig: function (node, call_back) {
        ...
        var iq = strophe_js.$iq({from:this.jid, to:this.service, type:'get', id:iqid})
          .c('pubsub', {xmlns:strophe_js.Strophe.NS.PUBSUB_OWNER})
    ...
    */
    subscribe: function(node, options, event_cb, success, error, barejid) {
        ...
        var jid = this.jid;
        if(barejid)
            jid = strophe_js.Strophe.getBareJidFromJid(jid);

        var iq = strophe_js.$iq({from:this.jid, to:this.service, type:'set', id:iqid})
          .c('pubsub', { xmlns:strophe_js.Strophe.NS.PUBSUB })
          .c('subscribe', {'node':node, 'jid':jid});
        if(options) {
            iq.up().c('options').form(strophe_js.Strophe.NS.PUBSUB_SUBSCRIBE_OPTIONS, options);
        }
    ...
    */
    ...
    getAffiliations: function(node, call_back) {
        ...
        var attrs = {}, xmlns = {'xmlns':strophe_js.Strophe.NS.PUBSUB};
        if (node) {
            attrs.node = node;
    ...
    publishAtom: function(node, atoms, call_back) {
        ...
        for (i = 0; i < atoms.length; i++) {
            ...
            entries.push({
                data: strophe_js.$build("entry", { xmlns:strophe_js.Strophe.NS.ATOM })
                        .children(atom).tree(),
